...
static float generate_trapezoid_freq(DMA_DB_t *dma_db, uint32_t pulse_index)
{
    ...
    uint32_t accel_pulses = dma_db->accel_pulses;
    uint32_t decel_pulses = dma_db->decel_pulses;
    uint32_t cruise_pulses = total_pulses - accel_pulses - decel_pulses;
    uint32_t max_rpm = dma_db->max_rpm;
    uint32_t pulses_per_rev = dma_db->pulses_per_rev;

    // 频率参数（单位：Hz）
    // uint16_t PSC = dma_db->htim->Instance->PSC;
...
void dma_db_fill_buffer(DMA_DB_t *dma_db)
{
    ...
    for (uint16_t i = 0; i < half_size; i++)
    {
        ...
        {
            // uint32_t ccr32 = dma_db->g_last_accum + 4;
            // dma_db->g_last_accum = ccr32;

            uint32_t ccr32 = dma_db_generate_t_ccr(dma_db, pulse_index);
...
uint32_t dma_db_generate_t_ccr(DMA_DB_t *dma_db, uint32_t transfer_index)
{
    ...
    // // 翻转模式下，传输两次才是个完整的脉冲
    // dma_db->g_last_accum;
    // dma_db->step_delay = 40;
    // uint32_t tmp = dma_db->g_last_accum + dma_db->step_delay / 2;
    // dma_db->g_last_accum = tmp;
    ...
    // if ((step_index + 1) % 2 == 0)
    // {
    //     dma_db->g_last_accum += (uint64_t)dma_db->step_delay;
    //     return (uint32_t)(dma_db->g_last_accum & 0xFFFFFFFFUL);
    // }
    ...
    // // double max_freg = dma_db->max_rpm * dma_db->pulses_per_rev / 60; // 电机最高转速时，走一步所需的时间
    // // double motor_min_tick = 1.0 / max_freg;                                              // 0.000004
    // // double min_ccr = motor_min_tick / TICK_HZ;                                           // 40

    // dma_db->step_delay = period_sec / TICK_HZ;
    ...
    // uint32_t ccr = (uint32_t)(dma_db->g_last_accum & 0xFFFFFFFFUL);
    // //
    // printf("step_index:%lu ,freq_hz: %.2f ccr: %lu,step_delay:%u\n", step_index, freq_hz, ccr, dma_db->step_delay);
    // return ccr;
}