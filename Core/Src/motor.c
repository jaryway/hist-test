...
{
    // motor_create_t_ctrl_param(motor, dma_db->total_pulses, dma_db->accel_pulses, dma_db->decel_pulses, motor->max_rpm);
    // HAL_TIM_OC_Start_DMA(motor->htim, motor->tim_channel, (uint32_t *)dma_db->dma_buffer, dma_db->buffer_size);
 ...
 * @retval      无
 */
void motor_create_t_ctrl_param(Motor_t *motor, int32_t pulses, uint32_t accel, uint32_t decel, uint32_t speed)
{
    // __IO uint16_t tim_count; /* 达到最大速度时的步数*/
    __IO uint32_t max_s_lim; /* 必须要开始减速的步数（如果加速没有达到最大速度）*/
    __IO uint32_t accel_lim;
    if (motor->motion_sta != STOP) /* 只允许步进电机在停止的时候才继续*/
        return;
    ...
    // printf("step_delay: %ld ,min_delay:%ld, max_s_lim: %lu\r\n",
    //        motor->step_delay, motor->min_delay, max_s_lim);
...
void calc_step_delay(Motor_t *motor)
{
    ...
    // if (motor->___i == 2) /* 2次，说明已经输出一个完整脉冲 */
    // {
